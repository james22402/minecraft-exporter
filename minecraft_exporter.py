...
class MinecraftCollector(object):
    ...
    def update_metrics_for_player(self,uuid):
        ...
        blocks_mined        = CounterMetricFamily('blocks_mined','Blocks a Player mined', labels={'player': name})
        blocks_picked_up    = CounterMetricFamily('blocks_picked_up','Blocks a Player picked up', labels={'player': name})
        player_deaths       = CounterMetricFamily('player_deaths','How often a Player died', labels={'player': name})
        player_jumps        = CounterMetricFamily('player_jumps','How often a Player has jumped', labels={'player': name})
        cm_traveled         = CounterMetricFamily('cm_traveled','How many cm a Player traveled, whatever that means', labels={'player': name})
        player_xp_total     = CounterMetricFamily('player_xp_total',"How much total XP a player has", labels={'player': name})
        player_current_level= CounterMetricFamily('player_current_level',"How much current XP a player has", labels={'player': name})
        player_food_level   = CounterMetricFamily('player_food_level',"How much food the player currently has", labels={'player': name})
        player_health       = CounterMetricFamily('player_health',"How much Health the player currently has", labels={'player': name})
        player_score        = CounterMetricFamily('player_score',"The Score of the player", labels={'player': name})
        entities_killed     = CounterMetricFamily('entities_killed',"Entities killed by player", labels={'player': name})
        damage_taken        = CounterMetricFamily('damage_taken',"Damage Taken by Player", labels={'player': name})
        damage_dealt        = CounterMetricFamily('damage_dealt',"Damage dealt by Player", labels={'player': name})
        blocks_crafted      = CounterMetricFamily('blocks_crafted',"Items a Player crafted", labels={'player': name})
        player_playtime     = CounterMetricFamily('player_playtime',"Time in Minutes a Player was online", labels={'player': name})
        player_advancements = CounterMetricFamily('player_advancements', "Number of completed advances of a player", labels={'player': name})
        player_slept        = CounterMetricFamily('player_slept',"Times a Player slept in a bed", labels={'player': name})
        player_quests_finished = CounterMetricFamily('player_quests_finished', 'Number of quests a Player has finished',  labels={'player': name})
        player_used_crafting_table = CounterMetricFamily('player_used_crafting_table',"Times a Player used a Crafting Table", labels={'player': name})
